...
# Create the diagonal matrix
    ...
    #def set_random_initial_state(self):
        # self.r_I_init[2] = 50
        #self.r_I_init[0:2] = np.random.uniform(-30, 30, size=2)

        # self.v_I_init[2] = np.random.uniform(-10, -6)
        # self.v_I_init[0:2] = np.random.uniform(-0.5, -0.2, size=2) * self.r_I_init[0:2]

        # self.q_B_I_init = euler_to_quat((np.random.uniform(-20, 20),
        #                                  np.random.uniform(-20, 20),
        #                                  0))
        # self.w_B_init = np.deg2rad((np.random.uniform(-5, 5),
    ...
    def nondimensionalize(self):
        ...
        print('x_initial nondim' ,self.x_init)
        print('x fiinak nondim' ,self.x_final)
    ...
    #     """
    #     Initialize the trajectory using predefined waypoints (init, middle, final).

    #     :param X: Numpy array of states to be initialized (Nx x K)
    #     :param U: Numpy array of inputs to be initialized (Nu x K)
    #     :return: The initialized X and U
    #     """
    #     # Indices for the segments
    #     waypoint_index = K // 2  # Middle index for the waypoint

    #     for k in range(K):
    #         if k < waypoint_index:  # Segment from initial to waypoint
    #             alpha1 = (waypoint_index - k) / waypoint_index
    #             alpha2 = k / waypoint_index

    #             m_k = (alpha1 * self.x_init[0] + alpha2 * self.x_middle[0],)
    #             r_I_k = alpha1 * self.x_init[1:4] + alpha2 * self.x_middle[1:4]
    #             v_I_k = alpha1 * self.x_init[4:7] + alpha2 * self.x_middle[4:7]
    #             q_B_I_k = self.x_middle[7:11]  # Assuming constant quaternion for simplicity
    #             w_B_k = alpha1 * self.x_init[11:14] + alpha2 * self.x_middle[11:14]
                
    #         else:  # Segment from waypoint to final
    #             alpha1 = (K - k) / (K - waypoint_index)
    #             alpha2 = (k - waypoint_index) / (K - waypoint_index)

    #             m_k = (alpha1 * self.x_middle[0] + alpha2 * self.x_final[0],)
    #             r_I_k = alpha1 * self.x_middle[1:4] + alpha2 * self.x_final[1:4]
    #             v_I_k = alpha1 * self.x_middle[4:7] + alpha2 * self.x_final[4:7]
    #             q_B_I_k = self.x_final[7:11]  # Assuming constant quaternion for simplicity
    #             w_B_k = alpha1 * self.x_middle[11:14] + alpha2 * self.x_final[11:14]
                

    #         # Assign the computed state to X and U
    ...
    def rocket_dynamics(self,x, u):
        ...
        self.J_B = jnp.diag(jnp.array([J1, J2, J3]))
        self.J_B /= (self.m_scale * self.r_scale ** 2)  # 1
