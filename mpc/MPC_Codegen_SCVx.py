...
class MPC_gen:
    """
    A class for generating c code of a Model Predictive Controller (MPC) for tracking a SCVx reference trajectory.
    ...
    def __init__(
        ...
        for i in range(self.Nt + self.N_ext - 1):
            ...
            self.X_nominal[:, i] = self.compute_rk4(
                self.X_bar[:, i], self.U_bar[:, i]
            ).full().flatten()
...
"""
        ...
        U_Tlb_Par_1 = cp.Parameter((self.N_mpc - 1, self.Nu), name="U_Tlb_Par_1")
        U_Tlb_Par_2 = cp.Parameter((self.N_mpc - 1), name="U_Tlb_Par_2")

        U_ref_Par = cp.Parameter((self.Nu, self.N_mpc - 1), name="U_ref_Par")
        X_ref_Par = cp.Parameter(
            (self.Nx_MPC, self.N_mpc), name="X_ref_Par"
        )
        ...
        U_Tlb_Par_1_values = np.zeros((self.N_mpc - 1, self.Nu))
        U_Tlb_Par_2_values = np.zeros(self.N_mpc - 1)
        for i in range(self.N_mpc - 1):

            U_Tlb_Par_1_values[i, :] = U_bar[:, i] / np.linalg.norm(U_bar[:, i])
            U_Tlb_Par_2_values[i] = U_Tlb_Par_1_values[i, :] @ U_bar[:, i]
        U_Tlb_Par_1.value = U_Tlb_Par_1_values
        U_Tlb_Par_2.value = U_Tlb_Par_2_values
        ...
        for i in range(self.N_mpc - 1):
            cost += 0.5 * cp.quad_form(DeltaU[:, i], 0.0000001 * self.R)
        #   ui = U_bar[:,i] + DeltaU[:, i]
...
"""
        ...
        while time_sim < self.T_sim - 2 * self.h_mpc:
            ...
            self.x[:, idx + 1] = self.rocket_dynamics.rocket_dynamics_rk4_SIM_SCVx(
