...
class MPC_Tracker:
    def __init__(
        ...
        self.X_Tlb_Par_1 = np.zeros((self.N_mpc - 1, self.Nu))
        self.X_Tlb_Par_2 = np.zeros((self.N_mpc - 1))
    ...
    def solve(self, x, u, idx, time_sim):
        ...
        - Selects the correct slices of the reference trajectory based on the simulation time for both the state and control inputs.
        ...
        for i in range(self.N_mpc - 1):
            ...
            self.X_Tlb_Par_1[i, :] = X_bar_new[13:16, i] / np.linalg.norm(
                X_bar_new[13:16, i]
            )
            self.X_Tlb_Par_2[i] = self.X_Tlb_Par_1[i, :] @ X_bar_new[13:16, i]
        self.prob.param_dict["X_ref_Par"].value = np.array(X_bar_new)
        self.prob.param_dict["U_ref_Par"].value = np.array(U_bar_new)

        self.prob.param_dict["X_Tlb_Par_1"].value = np.array(self.X_Tlb_Par_1)
        self.prob.param_dict["X_Tlb_Par_2"].value = np.array(self.X_Tlb_Par_2)
        ...
        uu = self.rocket_dynamics.add_percentage_gaussian_noise(uu, 0.1)
